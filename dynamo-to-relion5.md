...
# Extract particles at bin4 for Relion
...
2. Use this star file to re-extract particles with warp:
    ...
    --output_star relion5_b4/Extract/bin4_box$BOXSIZE/particles.star \
    ...
    --2d \
    --output_processing relion5_b4/Extract/bin4_box$BOXSIZE/
    ```
...
# Extract particles at bin 1

2. Use this star file to re-extract particles with warp:

    ```shell
    # Set boxsize for extraction at bin 1
    BOXSIZE_BIN1=128
    BOXDIM=$(awk "BEGIN { print int($ANGPIX * $BOXSIZE_BIN1) }")
    # Export particles
    cd $ROOT/warp
    WarpTools ts_export_particles \
    --settings warp_tiltseries.settings \
    --input_star particles_dynamo_b2.star \
    --output_star relion5_b1/Extract/bin1_box$BOXSIZE_BIN1/particles.star \
    --coords_angpix $ANGPIX \
    ...
    --relative_output_paths \
    --2d \
    --output_processing relion5_b1/Extract/bin1_box$BOXSIZE_BIN1/
    ```
...
3. To do helical processing, Relion5 expects a ```_rnlAngleTilt``` of 90. The script ```warp_to_relion5.py``` rotates the Euler angle of the subtomograms such that ```_rnlAngleTilt``` is 90. Also, it converts the coordinates to be compatible with Relion5 (in Angstroms and measured from the center of the tomogram).
    ...
    cd $REPOSITORY
    python warp_to_relion5.py \
    --relionroot $ROOT/warp/relion5_b1 \
    --warp ../particles_dynamo_b2.star \
    --relion Extract/bin1_box$BOXSIZE_BIN1/particles.star \
    --tomo Extract/bin1_box$BOXSIZE_BIN1/particles_tomograms.star \
    --out Extract/bin1_box$BOXSIZE_BIN1/particles_rot90.star
    ```

4. Rescale one of the two halfmaps from dynamo bin4 to bin1.
First, convert one of the halfmaps to an mrc file:
    ```shell
    cd $ROOT/warp
    ANGPIX_BIN4=$(echo "$ANGPIX * 4" | bc)
    # Rescale to the new pixel size
    relion_image_handler --i dynamo_bin4_ite4_sym.mrc --o relion5_b1/dynamo_bin1_ite4_sym_box$BOXSIZE_BIN1.mrc --angpix $ANGPIX_BIN4 --rescale_angpix $ANGPIX --new_box $BOXSIZE_BIN1 --force_header_angpix $ANGPIX
    ```

# Generate a mask

```shell
relion_helix_toolbox --o mask.mrc --cyl_outer_diameter 200 --angpix 7.92 --boxdim 64 --sphere_percentage 0.6 --cylinder
```
